  ...
  if (root.nodes && root.nodes.length) {
    root.nodes.slice().forEach(child => {
      if (isVarFunction(child)) {
        ...
        const _child$nodes = child.nodes,
              propertyNode = _child$nodes[0],
              comma = _child$nodes[1],
              fallbacks = _child$nodes.slice(2);
...
var index = postcss.plugin('postcss-custom-properties', opts => {
  ...
  const customPropertiesPromise = getCustomPropertiesFromImports(importFrom); // make sure we work in synchronous mode if no asynchronous operations are requested
